...
//
// This file contains an example OpenCL kernel as part of the conv_simple.cc example. This assumes
...
__kernel void conv(const int sizeX, const int sizeY,
                   const __global float* src,
                   __constant float* coeff,
                   __global floatvec* dest) {

  // Thread identifiers
  const int gid_x = get_global_id(0); // From 0 to sizeX/WPTX-1
  const int gid_y = get_global_id(1); // From 0 to sizeY/WPTY-1
